...
public class X12Activity extends AppCompatActivity {

    private RadarChart radar;
    private List<Double> x12_list = new ArrayList<>();
    private static MainActivity m = new MainActivity();
    private Button x12_back;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_x12);
        radar = (RadarChart) findViewById(R.id.x12_Chart);
        x12_back = findViewById(R.id.x12_back);
        x12_back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(X12Activity.this, MainActivity.class);
    ...
    private void initData() {
        ...
        for(int i = 0;i < m.DataList.size(); i++){
            x12_list.add(m.DataList.get(i).getX12());
        }
    }
//    private void initData() {
//        x6_list.add(6370241);
...
//    }
    ...
    private void initRadar() {
        ...
        for(int i = 0;i < x12_list.size();i++){
            list.add(new RadarEntry(Float.parseFloat(String.valueOf(x12_list.get(i)))));
        }

        RadarDataSet radarDataSet=new RadarDataSet(list,"单位：1");
...
//        radarData.addDataSet(radarDataSet1);
        ...
        //大字的颜色（中心点和各顶点的连线）
        radar.setWebColor(Color.rgb(238,162,173));
        //所有五边形的颜色
        radar.setWebColorInner(Color.rgb(224,238,224));
        //整个控件的背景颜色
...
//        yAxis.setAxisMinimum(0);   //Y轴最小数值
        ...
        yAxis.setLabelCount(x12_list.size(),false);
