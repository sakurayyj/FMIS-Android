...
public class X10Activity extends AppCompatActivity {

    private BarChart barChart;
    private List<Double> x10_list = new ArrayList<>();
    private static MainActivity m = new MainActivity();
    private Button x10_back;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_x10);
        x10_back = findViewById(R.id.x10_back);
        x10_back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(X10Activity.this, MainActivity.class);
    ...
    private void initData() {
        ...
        for(int i = 0;i < m.DataList.size(); i++){
            x10_list.add(m.DataList.get(i).getX10());
        }
    }
    private void initBarChart() {
        barChart = findViewById(R.id.x10_Chart);
    ...
    private void setData() {
        ...
        for(int i = 0;i < x10_list.size(); i++){
            float d = x10_list.get(i).floatValue();
            barEntries1.add(new BarEntry(i,d));
        }
        BarDataSet barDataSet1 = new BarDataSet(barEntries1, "");
        barDataSet1.setValueTextColor(Color.rgb(	205,102,29)); // 值的颜色
        barDataSet1.setValueTextSize(10f); // 值的大小
        barDataSet1.setColor(Color.parseColor("#FF7F24")); // 柱子的颜色
...
//    }
    ...
    private void setAxis() {
        ...
        xAxis.setLabelCount(x10_list.size());  // 设置x轴上的标签个数
        xAxis.setTextSize(10f); // x轴上标签的大小
        // 设置x轴显示的值的格式
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                if ((int) value < x10_list.size()) {
